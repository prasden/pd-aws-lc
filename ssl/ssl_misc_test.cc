...
TEST(SSLTest, QuietShutdown) {
  ...
  // unexpected transport EOF, not a close_notify.
  char buf[1];
  ...
  EXPECT_EQ(ERR_GET_REASON(err), SSL_R_UNEXPECTED_EOF_WHILE_READING);
...
// Returns a read |BIO| method that mimics a socket: it returns 0 on read to
// signal a transport EOF and does not implement BIO_CTRL_EOF.
static bssl::UniquePtr<BIO_METHOD> EOFReadBIOMethod() {
  bssl::UniquePtr<BIO_METHOD> method(BIO_meth_new(0, nullptr));
  if (!method ||
      !BIO_meth_set_create(method.get(),
                           [](BIO *b) -> int {
                             BIO_set_init(b, 1);
                             return 1;
                           }) ||
      !BIO_meth_set_read(method.get(),
                         [](BIO *, char *, int) -> int { return 0; }) ||
      !BIO_meth_set_ctrl(method.get(),
                         [](BIO *, int, long, void *) -> long { return 0; })) {
    return nullptr;
  }
  return method;
}

// Test that, by default, an unexpected transport EOF (the peer closing without
// a close_notify) is reported as |SSL_ERROR_SSL| with
// |SSL_R_UNEXPECTED_EOF_WHILE_READING| on the error queue, matching OpenSSL 3.
TEST(SSLTest, UnexpectedEOF) {
  ...
  bssl::UniquePtr<BIO_METHOD> method = EOFReadBIOMethod();
  ASSERT_TRUE(method);
  ...
  for (SSL *ssl : {client.get(), server.get()}) {
    ...
    EXPECT_EQ(ret, -1);
    EXPECT_EQ(SSL_get_error(ssl, ret), SSL_ERROR_SSL);
    uint32_t err = ERR_get_error();
    EXPECT_EQ(ERR_GET_LIB(err), ERR_LIB_SSL);
    EXPECT_EQ(ERR_GET_REASON(err), SSL_R_UNEXPECTED_EOF_WHILE_READING);

    // The read half is now in an error state, so the error is replayed on
    // subsequent reads.
...
// |SSL_ERROR_ZERO_RETURN|, for both the client and the server.
TEST(SSLTest, IgnoreUnexpectedEOF) {
  ...
  // shutdown, |SSL_ERROR_ZERO_RETURN|.
  char buf[1];
  int ret = SSL_read(client.get(), buf, sizeof(buf));
  EXPECT_EQ(ret, 0);
